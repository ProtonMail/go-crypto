...
func (e *Entity) NewForwardingEntity(
	...
	if _, err = e.VerifyPrimaryKey(now, config); err != nil {
		return nil, nil, err
	}
	...
	for _, forwarderSubKey := range e.Subkeys {
		...
		forwarderSubKeySelfSig, err := forwarderSubKey.Verify(now, config)
		...
		if forwarderSubKey.PrivateKey == nil {
			return nil, nil, errors.InvalidArgumentError("forwarder subkey has no private key")
		}
