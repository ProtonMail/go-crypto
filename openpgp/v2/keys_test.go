...
func TestGenerateRSAKeyPLessThanQ(t *testing.T) {
	...
	checkKey := func(t *testing.T, key *rsa.PrivateKey, err error) {
		t.Helper()
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("enforceRSAPrimeOrder", func(t *testing.T) {
	...
	t.Run("generateRSAKeyWithPrimes", func(t *testing.T) {
		key, err := generateRSAKeyWithPrimes(rand.Reader, 2, 2048, nil)
		checkKey(t, key, err)
	})
}
